var pendingServerMap = {}

function createCowboys(klass, serverOpts) {
...
function addServersAsync(spool) {
  ...
  if (sids.length > 0) {
    ...
    if (lid) {
      ...
      spool[lid].addServer({ newServer: sid }, function (result) {
...
function validateState(spool) {
  ...
  if (leaderCnt !== 1) {
    throw new Error("FAIL: Found " + leaderCnt + " leaders, expected 1")
  }
